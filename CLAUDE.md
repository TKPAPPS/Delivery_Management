...
Orders are turned into deliveries via **`POST /api/deliveries/from-orders`** `{ order_ids: string[] }`: it creates one **draft** `delivery_card`, then **groups the selected orders by customer name** (`lower(trim(customer_name))`) so several orders for the same customer become **one** `delivery_customer` (their `order_value`s are summed and their sale orders / line items combined). It maps distinct `sale_order_number`s → `customer_sale_orders` and each order line → `extra_delivery_items`, marks each order `status='assigned'`, sets `orders.delivery_card_id` (card link) **and `orders.delivery_customer_id`** (the FK that ties every order of a combined customer to its `delivery_customer` row; `on delete set null`). Unload/move/delete enumerate a customer's orders via `delivery_customer_id` (a customer may span several orders), falling back to the legacy single `delivery_customers.order_id` for rows created before the link existed.
...
- **Move a single sale order** (`POST /api/sale-orders/[id]/move`, `id` = the `customer_sale_orders` chip id) peels **one SO** off a combined customer to another card, leaving the rest of that customer in place. Both routes share `resolveDestinationCard()` (the 4 destinations: move / create_card / planning_queue / delayed) in `src/lib/customer-moves.ts`; the split itself is `moveSaleOrder()`. Since SO chips have no FK to `orders`, the chip's `sale_order_number` is matched to `orders.odoo_order_ref` (scoped by `delivery_customer_id`), and the order's `extra_delivery_items` are re-attributed from its `order_lines` **one-per-line** by `(item_name, quantity)` (so duplicate rows like two "Delivery Service (1)" split correctly). `order_value` is adjusted by the moved order's `amount_total`. If the customer has only that one SO it degrades to a whole-customer move. UI: a per-chip move button in `CustomerSection.tsx` reusing `UnloadCustomerModal` in `saleOrder` scope.
