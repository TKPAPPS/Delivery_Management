...
import { Mail, Plus, Edit, RefreshCw } from 'lucide-react';
...
export default function CourierCompaniesPage() {
  ...
  const [editItem, setEditItem] = useState<CourierCompany | null>(null);
  ...
  const openEdit = (c: CourierCompany) => { setEditItem(c); setName(c.name); setModalOpen(true); };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const url = editItem ? `/api/courier-companies/${editItem.id}` : '/api/courier-companies';
  ...
  const toggleActive = async (c: CourierCompany) => {
    try {
      await fetch(`/api/courier-companies/${c.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ active: !c.active }) });
    ...
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold text-black">Courier Companies</h1>
        </div>
        <div className="flex gap-2">
          <Button variant="ghost" size="sm" onClick={fetch_} loading={loading}><RefreshCw className="w-4 h-4" /></Button>
          <Button onClick={openCreate}><Plus className="w-4 h-4" /> Add Courier</Button>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        ...
        {!loading && companies.length === 0 && <div className="text-center py-12 text-slate-400">No couriers yet</div>}
      </div>

      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'Edit Courier' : 'Add Courier'} size="sm">
        <form onSubmit={handleSave} className="space-y-3">
          <Input label="Name *" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Kerry Express" />
          <div className="flex gap-3 justify-end pt-2">
            <Button type="button" variant="secondary" onClick={() => setModalOpen(false)} disabled={saving}>Cancel</Button>
            <Button type="submit" loading={saving}>{editItem ? 'Save Changes' : 'Add Courier'}</Button>
          </div>
        </form>
      </Modal>
    </div>
